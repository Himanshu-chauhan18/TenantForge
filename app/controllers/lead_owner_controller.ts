...
import LeadOwnerService from '#services/lead_owner_service'
import { leadOwnerValidator } from '#validators/lead_owner_validator'
...
export default class LeadOwnerController {
  ...
  async bulk({ request, response, session }: HttpContext) {
    ...
    await LeadOwner.query().whereIn('id', ids as number[]).update({ status })
