...
export default function LeadsIndex({ owners, assignments }: Props) {
  ...
  function handleTab(key: string) {
    setSelected([])
    router.get('/leads', { tab: key }, { preserveState: true, replace: true })
  }
  ...
  function navigate(params: Record<string, string | number | undefined>) {
    ...
    router.get('/leads', p, { preserveState: true, replace: true })
  }
  ...
  function handleBulkDirect(op: 'activate' | 'deactivate') {
    ...
    router.post('/leads/bulk', { ids: selected, operation: op }, {
  ...
  function handleEdit() {
    ...
    setProcessing(true)
    router.put(`/leads/${editTarget.id}`, {
  ...
  const assignCols: DTColumn<Assignment>[] = [
    {
      ...
      render: (row) => (
        <a href={`/organizations/${row.id}`} style={{ textDecoration: 'none', display: 'block' }}>
