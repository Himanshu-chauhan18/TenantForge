...
export default function OrganizationsIndex({ orgs, leadOwners }: Props) {
  ...
  function navigate(params: Record<string, string | number | undefined>) {
    ...
    router.get('/orgbuilder/organizations', p, { preserveState: true, replace: true })
  }
  ...
  const exportUrl = useMemo(() => {
    ...
    return `/orgbuilder/organizations/export?${params.toString()}`
  ...
  function handleBulkDirect(op: string) {
    ...
    router.post('/orgbuilder/organizations/bulk', { ids: selected, operation: op }, {
  ...
  function submitExtendPlan() {
    ...
    router.post('/orgbuilder/organizations/bulk', { ids: selected, operation: 'extend_plan', payload: { planEnd: extendDate } }, {
  ...
  function submitExtendLimit() {
    ...
    if (!n || n < 1) return
    router.post('/orgbuilder/organizations/bulk', { ids: selected, operation: 'extend_user_limit', payload: { userLimit: n } }, {
  ...
  function submitAssignLead() {
    ...
    router.post('/orgbuilder/organizations/bulk', { ids: selected, operation: 'assign_lead', payload: { leadOwnerId: Number(assignLeadId) } }, {
      onSuccess: () => { setSelected([]); setBulkModal(null); setAssignLeadId('') },
    })
  }

  function handleDelete(id: number) {
    router.delete(`/orgbuilder/organizations/${id}`)
      ...
      render: (org) => (
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          {org.isArchived ? (
            ...
            <Link
              href={`/orgbuilder/organizations/${org.id}/edit`}
              title="Edit"
              style={{
                display: 'inline-flex', alignItems: 'center', gap: 4,
                padding: '4px 10px', borderRadius: 7, fontSize: '.72rem', fontWeight: 600,
    ...
    <>
      ...
      <div className="ph">
        ...
        <div className="ph-right">
          ...
          {/* Add Organization */}
          <Link
            href="/orgbuilder/organizations/create"
            className="btn btn-p"
      ...
      <div className="card">
        ...
        <DataTable<Org>
          ...
          emptyAction={
            <Link href="/orgbuilder/organizations/create" className="btn btn-p btn-sm" style={{ display: 'inline-flex' }}>
            ...
            <button
              ...
              onClick={() => {
                if (unarchiveConfirm !== null) {
                  router.post('/orgbuilder/organizations/bulk', { ids: [unarchiveConfirm], operation: 'unarchive' })
